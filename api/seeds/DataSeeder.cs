...
namespace RealEstateHubAPI.seeds
{
    public static class DataSeeder
    {
        public static void SeedData(ApplicationDbContext context, bool force = false)
        {
            ...
            // Seed Messages (chỉ seed nếu đã có Users và Posts)
            if ((force || !context.Set<Message>().Any()) && context.Users.Any() && context.Posts.Any())
            {
                var userNormal = context.Users.FirstOrDefault(u => u.Email == "user1@example.com");
                var userPro1 = context.Users.FirstOrDefault(u => u.Email == "user2@example.com");
                var userPro3 = context.Users.FirstOrDefault(u => u.Email == "user3@example.com");
                ...
                if (userNormal != null && userPro1 != null && userPro3 != null && posts.Count > 0)
                {
                    ...
                    // Seed messages cho các posts có sẵn (kiểm tra index tồn tại)
                    if (posts.Count > 0)
                    {
                        messages.Add(new Message
                        {
                            SenderId = userNormal.Id,
                            ReceiverId = userPro1.Id,
                            PostId = posts[0].Id,
                            Content = "Xin chào, tôi quan tâm đến biệt thự này. Có thể cho tôi biết thêm thông tin không?",
                            SentTime = DateTime.Now.AddDays(-5).AddHours(-2)
                        });
                        messages.Add(new Message
                        {
                            SenderId = userPro1.Id,
                            ReceiverId = userNormal.Id,
                            PostId = posts[0].Id,
                            Content = "Chào bạn, biệt thự này còn đang bán. Bạn có muốn xem trực tiếp không?",
                            SentTime = DateTime.Now.AddDays(-5).AddHours(-1)
                        });
                        messages.Add(new Message
                        {
                            SenderId = userNormal.Id,
                            ReceiverId = userPro1.Id,
                            PostId = posts[0].Id,
                            Content = "Có, tôi muốn xem vào cuối tuần này. Có được không?",
                    ...
                    if (posts.Count > 3)
                    {
                        messages.Add(new Message
                        {
                            SenderId = userPro3.Id,
                            ReceiverId = userPro1.Id,
                            PostId = posts[3].Id,
                            Content = "Đất nền này còn bán không? Giá có thể thương lượng không?",
                            SentTime = DateTime.Now.AddDays(-2).AddHours(-3)
                        });
                        messages.Add(new Message
                        {
                            SenderId = userPro1.Id,
                            ReceiverId = userPro3.Id,
                            PostId = posts[3].Id,
                            Content = "Vâng, đất nền vẫn còn. Giá có thể thương lượng một chút. Bạn muốn xem đất không?",
                            SentTime = DateTime.Now.AddDays(-2).AddHours(-2)
                        });
                    }
                    if (posts.Count > 5)
                    {
                        messages.Add(new Message
                        {
                            ...
                            ReceiverId = userPro3.Id,
                            PostId = posts[5].Id,
                            Content = "Văn phòng này cho thuê bao lâu? Có thể thuê ngắn hạn không?",
                            SentTime = DateTime.Now.AddDays(-1).AddHours(-5)
                        });
                        messages.Add(new Message
                        {
                            SenderId = userPro3.Id,
                            ReceiverId = userNormal.Id,
                            PostId = posts[5].Id,
                            Content = "Có thể thuê tối thiểu 6 tháng. Bạn có muốn xem văn phòng không?",
                            SentTime = DateTime.Now.AddDays(-1).AddHours(-4)
                        });
                    }
